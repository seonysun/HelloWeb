...
<%-- 
1. 자바스크립트
	1) 역할 : 브라우저에서 이벤트 기능 처리 (클릭, 변경, 마우스, 키보드 명령)
		- 정적 -> 동적 화면
	...
	2) 사용법
		...
		- 외부 스크립트 : .js 파일, 여러개 파일에서 사용 가능 -> 라이브러리
	...
	4) 출력
		- alert() : 메시지창, Modal
		- console.log() : 디버그, 콘솔창에 출력
		- document.write() : 브라우저 화면에 출력
		- 태그명.innerHTML="" : Ajax, VueJS, ReactJS

2. 변수
	1) 변수 선언
		- 변수의 데이터형이 없다 
			-> let/var로 통일, 값에 따라서 데이터형 자동으로 변경
		- 하나의 변수의 데이터형 계속해서 변경해서 사용할 수도 있음
		- 식별자 : 규칙 자바와 동일
			- 키워드 사용 불가
			- 특수문자 사용 가능(_, $)
			- 숫자로 시작할 수 없음
			...
			- 대소문자 구별
				- 생성자는 대문자 사용
				- 변수, 메소드, 함수는 소문자 사용
				cf. 함수 : 단독으로 사용, 코드 집합, 변수의 일종
					메소드 : 다른 식별자와 함께 사용, 객체에 소속된 함수
