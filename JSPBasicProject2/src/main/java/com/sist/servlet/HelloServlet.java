 ...
 */
@WebServlet("/hello.do")
			//호출(어노테이션) -> 확장자, 파일명 등은 자유롭게 설정 가능 ex. /HelloServlet /*.daum /*.nhn
/* 서블릿 호출
 * 	1) 브라우저에서 요청 
...
public class HelloServlet extends HttpServlet {
	...
	//init() 생성자 역할 -> 멤버변수 초기화, 자동 로그인, 파일 읽기
	public void init(ServletConfig config) throws ServletException {
		System.out.println("서블릿 초기화 메소드 호출..");
	}
	//destroy() 자동 메모리 해제 -> 브라우저 종료, 파일 이동, 새로고침
	public void destroy() {
	}
	
	//service() : 브라우저 화면에 출력하는 메소드
	/* HTML : 데이터 전송 -> 서버가 받아서 처리
	 * 	- GET : URL 뒤에 데이터 묶어서 전송
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=response.getWriter();
						//요청한 브라우저 정보(IP)
			//PrintWriter -> println() 메소드 -> 응답 데이터 전달(브라우저에 전송)
